...
namespace ConsoleApp1
{
    internal class Program{  
        ...
        public static int ChooseProduct(Seller[] sellers, int sellerIndex) {
            if(sellers[sellerIndex].GetProductArray().Length==0)
            {
                Console.WriteLine("No Products to choose from.");
                return -1;
            }
            for (int i = 0; i < sellers[sellerIndex].GetProductArray().Length; i++) {
                Console.Write($"{i + 1}. ");
        ...
        public static int ChooseBuyer(Buyer[] buyers) {
            if (buyers.Length == 0)
            {
                Console.WriteLine("No Buyers to choose from.");
                return -1;
            }
